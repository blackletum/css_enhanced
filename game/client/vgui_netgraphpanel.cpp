...
void CNetGraphPanel::DrawTextFields( int graphvalue, int x, int y, int w, netbandwidthgraph_t *graph, cmdinfo_t *cmdinfo )
{
	...
	Q_snprintf( sz, sizeof( sz ), "lerp: %5.1f ms", TICKS_TO_TIME( GetClientInterpolationAmountInTicks() ) * 1000.0f );

	int interpcolor[ 3 ] = { (int)GRAPH_RED, (int)GRAPH_GREEN, (int)GRAPH_BLUE }; 
	float flInterp = TICKS_TO_TIME( GetClientInterpolationAmountInTicks() );
	if ( flInterp > 0.001f )
	{
