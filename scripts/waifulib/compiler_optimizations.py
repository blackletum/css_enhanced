...
try:
    from fwgslib import get_flags_by_type, get_flags_by_compiler
except:
    from waflib.extras.fwgslib import get_flags_by_type, get_flags_by_compiler
...
CFLAGS = {
    ...
    "nooptimize": {"msvc": ["/Od", "/MDd"], "default": ["-O0"]},
}

LTO_CFLAGS = {"msvc": ["/GL"], "gcc": ["-flto"], "clang": ["-flto"]}

LTO_LINKFLAGS = {"msvc": ["/LTCG"], "gcc": ["-flto"], "clang": ["-flto"]}
...
def configure(conf):
    ...
    if conf.options.BUILD_TYPE.startswith("fast"):
        Logs.warn(
            "WARNING: '%s' build type should not be used in release builds",
            conf.options.BUILD_TYPE,
        )
...
def get_optimization_flags(conf):
    ...
    """
    linkflags = conf.get_flags_by_type(
        LINKFLAGS, conf.options.BUILD_TYPE, conf.env.COMPILER_CC, conf.env.CC_VERSION[0]
    )

    cflags = conf.get_flags_by_type(
        CFLAGS, conf.options.BUILD_TYPE, conf.env.COMPILER_CC, conf.env.CC_VERSION[0]
    )
